...
const sql = readFileSync(
  path.resolve(process.cwd(), 'supabase/migration_ring_labor_profiles_2026.sql'),
  'utf8'
);
...
async function main() {
  ...
  try {
    ...
    const profiles = rows[0]?.settings_values?.jewelry_setting_metal_profiles as
      | { ring?: { labor_rates?: Record<string, number> } }
      | undefined;
    console.log('Ring labor profile seeded:', profiles?.ring?.labor_rates ?? {});
