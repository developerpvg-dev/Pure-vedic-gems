...
import {
  buildPendantRecords,
  extractPendantImages,
  generatePendantSqlSeed,
  parsePendantDesignsFromFile,
} from './parse-pendant-designs';
...
async function main() {
  ...
  const publicDir = path.resolve('public', 'pendant-designs');
  const outJson = path.resolve('scripts', 'legacy-import', 'pendant-designs', 'parsed-designs.json');
  const outSql = path.resolve('supabase', 'migration_pendant_designs_2026.sql');

  if (!fs.existsSync(xlsxPath)) {
    throw new Error(`Excel file not found: ${xlsxPath}`);
  }

  console.log('Parsing pendant designs from Excel...');
  const designs = parsePendantDesignsFromFile(xlsxPath);
  console.log(`Found ${designs.length} pendant designs`);

  console.log('Extracting design images...');
  const imageMap = extractPendantImages(xlsxPath, publicDir);
  console.log(`Copied ${imageMap.size} images to public/pendant-designs/`);

  const records = buildPendantRecords(designs, imageMap);
  const sql = generatePendantSqlSeed(records);
  ...
  const summary = {
    ...
    withDiamondNotes: records.filter((r) => r.description?.toLowerCase().includes('diamond')).length,
  ...
  console.log('\nUpserting pendant designs (preserves existing IDs referenced by orders)...');
  const { updated, inserted } = await upsertJewelryDesignRecords(supabase, records);
  console.log(`Updated ${updated}, inserted ${inserted} pendant designs`);
}
