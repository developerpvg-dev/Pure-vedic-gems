/**
 * idols-jewellery/04-media.ts
 *
 * Enumerate idol/jewellery attachment IDs from
 * legacy_import.stg_idols_jewellery_products, download the legacy images,
 * convert to WebP, and upload to Supabase Storage under the idols-jewellery/
 * prefix. Mirrors leftover/05-media.ts and reuses the shared
 * legacy_import.stg_media_url_map (sha256 dedup) so any image already pulled by
 * an earlier phase is reused.
 *
 * Usage:
 *   npx tsx scripts/legacy-import/idols-jewellery/04-media.ts --enumerate                 (dry-run)
 *   npx tsx scripts/legacy-import/idols-jewellery/04-media.ts --enumerate --write --write-prod
 *   npx tsx scripts/legacy-import/idols-jewellery/04-media.ts --download  --write --write-prod
 */
...
const DOWNLOAD_CONCURRENCY = Math.max(1, Math.min(12, Number(process.env.LEGACY_IMPORT_DOWNLOAD_CONCURRENCY ?? '8') || 1));
const STORAGE_PREFIX = 'idols-jewellery';
// The legacy host throttles image-body transfer to a few KB/s, so allow a long
// per-attempt window. Throttling appears per-connection, so concurrency helps
// aggregate throughput (see LEGACY_IMPORT_DOWNLOAD_CONCURRENCY).
...
async function fetchWithRetry(legacyUrl: string): Promise<Buffer> {
  ...
  for (let attempt = 1; attempt <= FETCH_MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      if (!response.ok) {
        // 404/410 are permanent; do not retry.
        if (response.status === 404 || response.status === 410) throw new Error(`HTTP ${response.status}`);
...
async function loadCandidates(client: Client): Promise<MediaCandidate[]> {
  const products = await client.query(`SELECT legacy_thumbnail_url, legacy_image_urls FROM legacy_import.stg_idols_jewellery_products`);
...
async function enumerate(client: Client, flags: CliFlags): Promise<void> {
  ...
  console.log(`\nstg_media_url_map for idols/jewellery after upsert (${processed} processed):`);
...
async function download(client: Client, flags: CliFlags, supaUrl: string, supaKey: string): Promise<void> {
  ...
  console.log(`Pending idols/jewellery rows: ${pending.rows.length}`);
  ...
  async function worker() {
    while (queue.length > 0) {
      ...
      if (done % 20 === 0 || done === pending.rows.length) {
        console.log(`  processed ${done}/${pending.rows.length}  ok=${ok} dup=${duplicates} failed=${failed}`);
      }
    }
  }
...
async function processOne(client: Client, supabase: SupabaseClient, legacyUrl: string, attachmentId: number): Promise<'ok' | 'duplicate'> {
  const bytes = await fetchWithRetry(legacyUrl);
  ...
  const key = `${STORAGE_PREFIX}/${sha.slice(0, 2)}/${sha}.webp`;
