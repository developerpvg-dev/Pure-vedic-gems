/**
 * upratna/07-verify.ts
 *
 * DB-side verification gates for the Upratna legacy migration.
 */
...
async function main() {
  ...
  console.log(`Verifying Upratna migration on ${dbHost}\n`);
  ...
  try {
    const rowParity = await client.query(`
      SELECT
        (SELECT count(*)::int FROM legacy_import.stg_upratna_products) AS staged,
        (SELECT count(*)::int
           FROM public.products p
           JOIN legacy_import.stg_upratna_products s ON s.legacy_woo_id=p.legacy_woo_id) AS promoted,
        (SELECT count(*)::int
           FROM legacy_import.stg_upratna_products s
    ...
    const duplicates = await client.query(`
      SELECT count(*)::int AS n FROM (
        ...
        FROM public.products p
        JOIN legacy_import.stg_upratna_products s ON s.legacy_woo_id=p.legacy_woo_id
    ...
    const categories = await client.query(`
      SELECT
        (SELECT count(DISTINCT category_slug)::int FROM legacy_import.stg_upratna_categories) AS staged,
        (SELECT count(*)::int FROM public.gem_categories g JOIN legacy_import.stg_upratna_categories c ON c.category_slug=g.slug WHERE g.type='upratna' AND g.is_active) AS gem,
        (SELECT count(*)::int FROM public.product_categories pc JOIN legacy_import.stg_upratna_categories c ON c.category_slug=pc.slug WHERE pc.is_active) AS catalog`);
    results.push({
    ...
    const media = await client.query(`
      WITH expected_images AS (
        ...
        LEFT JOIN legacy_import.stg_media_url_map m
         ...
         AND m.storage_path LIKE 'upratna/%'
    ...
    const assignments = await client.query(`
      SELECT
        (SELECT count(*)::int FROM legacy_import.stg_upratna_products) AS total,
    ...
    const options = await client.query(`
      SELECT
        ...
        count(*) filter (where r.jewelry_design_enabled is not true)::int AS jewelry_design_disabled,
        count(*) filter (where p.configurator_enabled is not true)::int configurator_disabled
      FROM public.products p
      JOIN legacy_import.stg_upratna_products s ON s.legacy_woo_id=p.legacy_woo_id
      LEFT JOIN public.product_option_rules r ON r.product_id=p.id`);
    results.push({
      name: 'G6 option rules and configurator enabled',
      pass: options.rows[0].missing_rules === 0 && options.rows[0].energization_disabled === 0 && options.rows[0].jewelry_design_disabled === 0 && options.rows[0].configurator_disabled === 0,
      detail: `missing_rules=${options.rows[0].missing_rules} energization_disabled=${options.rows[0].energization_disabled} jewelry_design_disabled=${options.rows[0].jewelry_design_disabled} configurator_disabled=${options.rows[0].configurator_disabled}`,
    });

    const seo = await client.query(`
      SELECT
        count(*) filter (where p.meta_title is null or p.meta_title='')::int AS missing_title,
        count(*) filter (where length(p.meta_title) > 70)::int AS long_title,
        count(*) filter (where p.meta_description is null or p.meta_description='')::int AS missing_desc,
        count(*) filter (where length(p.meta_description) > 160)::int AS long_desc,
        count(*) filter (where p.canonical_url is null or p.canonical_url='' or p.canonical_url !~ '/shop/upratna/')::int AS bad_canonical
      FROM public.products p
      JOIN legacy_import.stg_upratna_products s ON s.legacy_woo_id=p.legacy_woo_id`);
    results.push({
      name: 'G7 SEO fields populated',
      pass: seo.rows[0].missing_title === 0 && seo.rows[0].long_title === 0 && seo.rows[0].missing_desc === 0 && seo.rows[0].long_desc === 0 && seo.rows[0].bad_canonical === 0,
      detail: `missing_title=${seo.rows[0].missing_title} long_title=${seo.rows[0].long_title} missing_desc=${seo.rows[0].missing_desc} long_desc=${seo.rows[0].long_desc} bad_canonical=${seo.rows[0].bad_canonical}`,
    });
