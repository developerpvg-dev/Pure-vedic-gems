/**
 * navratna/01-extract.ts
 *
 * Stream the legacy WP SQL dump and write JSONL files containing only the
 * rows Phase 1 needs. No Supabase writes happen here; this is a local
 * filesystem cache step.
 *
 * Strategy: small support tables (terms / term_taxonomy / term_relationships)
 * are loaded fully into memory in pass 1. Large tables (posts, postmeta) are
 * streamed in subsequent passes with row filters so memory stays bounded.
 *
 * Output (cwd-relative): scripts/legacy-import/_raw/navratna/
 *   terms.jsonl, term_taxonomy.jsonl, term_relationships.jsonl,
 *   posts.jsonl, postmeta.jsonl,
 *   attachments.jsonl, attachment_meta.jsonl, _summary.json
 *
...
if (!dumpPath) {
  throw new Error('Missing LEGACY_SQL_DUMP_PATH in env.');
}

const outDir = resolve(here, '..', '_raw', 'navratna');
mkdirSync(outDir, { recursive: true });

const ROOT_NAVRATAN_SLUGS = new Set(['navratan', 'navratna']);
...
async function main() {
  ...
  // ----- Resolve Navratan subtree -----
  const termById = new Map(terms.map((t) => [t.term_id, t]));
  const rootTax = termTax.find((tt) => {
    const term = termById.get(tt.term_id);
    return term ? ROOT_NAVRATAN_SLUGS.has(term.slug) : false;
  });
  if (!rootTax) {
    throw new Error(
      `Could not find Navratan root product_cat (tried slugs: ${[...ROOT_NAVRATAN_SLUGS].join(', ')}).`,
    );
  }
  const rootTerm = termById.get(rootTax.term_id)!;
  console.log(
    `  Navratan root: term_id=${rootTerm.term_id} ttid=${rootTax.term_taxonomy_id} slug="${rootTerm.slug}"`,
  );
  ...
  const queue: number[] = [rootTax.term_id];
  while (queue.length) {
    ...
    for (const child of childrenByParent.get(termId) ?? []) {
      queue.push(child.term_id);
    }
  }
  console.log(`  Navratan subtree: ${subtreeTermIds.size} terms`);

  // ----- Pass 2: term_relationships for subtree -----
  ...
  for await (const row of streamWpTable({
    ...
    termRels.push(t);
    trWriter.write(t);
    productIds.add(t.object_id);
  }
  await trWriter.close();
  console.log(
    `  wp_term_relationships (subtree): ${termRels.length}, candidate object_ids=${productIds.size}`,
  );
  ...
  // ----- Pass 4: wp_postmeta for product posts (also harvest attachment ids) -----
  ...
  // ----- Pass 5: attachments (by post_parent OR by referenced ID) -----
  ...
  // ----- Pass 6: postmeta for attachments -----
