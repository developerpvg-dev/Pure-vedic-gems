/**
 * leftover/05-media.ts
 *
 * Enumerate leftover attachment IDs, download legacy images, convert to WebP,
 * and upload to Supabase Storage under the leftover/ prefix. Mirrors the
 * upratna media pipeline. Uses the shared stg_media_url_map so any image already
 * downloaded by an earlier phase is reused via sha256 dedup.
 */
...
async function loadCandidates(client: Client): Promise<MediaCandidate[]> {
  const products = await client.query(`SELECT legacy_thumbnail_url, legacy_image_urls FROM legacy_import.stg_leftover_products`);
...
async function enumerate(client: Client, flags: CliFlags): Promise<void> {
  ...
  console.log(`\nstg_media_url_map for leftover after upsert (${processed} processed):`);
...
async function download(client: Client, flags: CliFlags, supaUrl: string, supaKey: string): Promise<void> {
  ...
  console.log(`Pending leftover rows: ${pending.rows.length}`);
...
async function processOne(client: Client, supabase: SupabaseClient, legacyUrl: string, attachmentId: number): Promise<'ok' | 'duplicate'> {
  ...
  const key = `leftover/${sha.slice(0, 2)}/${sha}.webp`;
