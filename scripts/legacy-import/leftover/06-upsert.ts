/**
 * leftover/06-upsert.ts
 *
 * Promote staged leftover products into public catalog tables. Branches by
 * `family`:
 *   - gemstone  -> upratna/navaratna shape (configurator, gemstone option rules)
 *   - rudraksha -> bead shape (energization, loose-only option rules, guide enrich)
 *
 * Replicates the upratna and rudraksha upsert conventions verbatim so migrated
 * rows are indistinguishable from their siblings. Idempotent via
 * ON CONFLICT (legacy_woo_id). After upsert, resolves the dedicated per-product
 * YouTube video (first gallery image carrying videolink_id) on both staging and
 * public, matching fix-dedicated-video-urls.ts.
 */
...
async function upsertGemstone(client: Client, row: StagedProduct, subCategoryId: string, parentId: string, media: MediaMap, batchId: string): Promise<void> {
  ...
  const upsert = await client.query(
    `INSERT INTO public.products (
     ...
     ON CONFLICT (legacy_woo_id) WHERE legacy_woo_id IS NOT NULL DO UPDATE SET
       ...
       price=EXCLUDED.price, price_per_carat=EXCLUDED.price_per_carat, compare_price=EXCLUDED.compare_price, currency=EXCLUDED.currency,
       carat_weight=EXCLUDED.carat_weight, ratti_weight=EXCLUDED.ratti_weight, shape=EXCLUDED.shape,
       ...
       quality_label=EXCLUDED.quality_label, color_description=EXCLUDED.color_description, clarity_description=EXCLUDED.clarity_description, treatment_summary=EXCLUDED.treatment_summary,
       origin_country=EXCLUDED.origin_country, origin_region=EXCLUDED.origin_region, origin_display=EXCLUDED.origin_display, dimensions_mm=EXCLUDED.dimensions_mm, composition=EXCLUDED.composition,
       recommendation_category_code=EXCLUDED.recommendation_category_code,
       certificate_number=EXCLUDED.certificate_number, certificate_lab=EXCLUDED.certificate_lab, certificate_status=EXCLUDED.certificate_status, certificate_file_url=EXCLUDED.certificate_file_url, certificate_display_enabled=EXCLUDED.certificate_display_enabled,
  ...
  await writeAssignments(client, productId, subCategoryId, parentId, row);
  await client.query(
...
async function upsertRudraksha(client: Client, row: StagedProduct, subCategoryId: string, parentId: string, media: MediaMap, batchId: string): Promise<void> {
  const { images, thumbnail_url } = buildImagesArray(row.legacy_image_urls, row.legacy_thumbnail_url, media);
  const warnings = Array.isArray(row.warnings) ? [...row.warnings] : [];
  if (images.length === 0) warnings.push('media: no uploaded images resolved');
  ...
  const upsert = await client.query(
    `INSERT INTO public.products (
     ...
     ON CONFLICT (legacy_woo_id) WHERE legacy_woo_id IS NOT NULL DO UPDATE SET
       ...
       mukhi_count=EXCLUDED.mukhi_count, ruling_deity=EXCLUDED.ruling_deity, deity=EXCLUDED.deity, planet=EXCLUDED.planet, chakra=EXCLUDED.chakra,
       short_desc=EXCLUDED.short_desc, description=EXCLUDED.description, clean_description=EXCLUDED.clean_description, legacy_html_description=EXCLUDED.legacy_html_description,
       images=EXCLUDED.images, thumbnail_url=EXCLUDED.thumbnail_url, video_url=EXCLUDED.video_url,
       in_stock=EXCLUDED.in_stock, stock_status=EXCLUDED.stock_status, stock_quantity=EXCLUDED.stock_quantity, availability_status=EXCLUDED.availability_status,
       meta_title=EXCLUDED.meta_title, meta_description=EXCLUDED.meta_description, meta_keywords=EXCLUDED.meta_keywords,
       canonical_url=EXCLUDED.canonical_url, seo_data=EXCLUDED.seo_data, legacy_seo=EXCLUDED.legacy_seo,
       certificate_number=EXCLUDED.certificate_number, certificate_lab=EXCLUDED.certificate_lab, certificate_status=EXCLUDED.certificate_status, certificate_display_enabled=EXCLUDED.certificate_display_enabled,
       xray_certificate_number=EXCLUDED.xray_certificate_number, xray_certified=EXCLUDED.xray_certified,
       product_type=EXCLUDED.product_type, price_mode=EXCLUDED.price_mode, configurator_enabled=EXCLUDED.configurator_enabled,
       rudraksha_type=EXCLUDED.rudraksha_type, bead_size_mm=EXCLUDED.bead_size_mm, bead_weight=EXCLUDED.bead_weight, mantra=EXCLUDED.mantra, energization_eligible=EXCLUDED.energization_eligible,
       legacy_sku=EXCLUDED.legacy_sku, legacy_slug=EXCLUDED.legacy_slug, legacy_permalink=EXCLUDED.legacy_permalink, legacy_status=EXCLUDED.legacy_status, legacy_created_at=EXCLUDED.legacy_created_at,
       import_batch_id=EXCLUDED.import_batch_id, import_warnings=EXCLUDED.import_warnings, legacy_data=EXCLUDED.legacy_data, updated_at=NOW()
     RETURNING id`,
    [
...
async function writeAssignments(client: Client, productId: string, subCategoryId: string, parentId: string, row: StagedProduct): Promise<void> {
...
const PARENT_BY_CATEGORY: Record<string, string> = { upratna: 'upratna', navaratna: 'navaratna', rudraksha: 'rudraksha' };
...
async function main() {
  ...
  try {
    ...
    for (const parentSlug of new Set(Object.values(PARENT_BY_CATEGORY))) {
      if (!categoryBySlug.get(parentSlug)) throw new Error(`public.product_categories has no active "${parentSlug}" parent row.`);
    }

    await client.query(
      `INSERT INTO public.product_import_batches (id, source, filename, status, started_at)
       VALUES ($1, 'legacy-migration', 'leftover-phase-1', 'running', NOW())
       ON CONFLICT (id) DO UPDATE SET status='running', started_at=NOW()`,
      [batchId],
    );

    const staged = await client.query<StagedProduct>(`SELECT * FROM legacy_import.stg_leftover_products ORDER BY legacy_woo_id`);
    ...
    for (const row of staged.rows) {
      ...
      try {
        ...
        const parentId = categoryBySlug.get(PARENT_BY_CATEGORY[row.category ?? ''] ?? '')!;
        if (row.family === 'rudraksha') {
          await upsertRudraksha(client, row, subCategoryId, parentId, media, batchId);
        } else {
          await upsertGemstone(client, row, subCategoryId, parentId, media, batchId);
        }
        processedRows++;
        await client.query('RELEASE SAVEPOINT row_sp');
      } catch (err) {
        ...
        if (!firstErrorPrinted) {
          ...
          console.error(`\n  FIRST ERROR legacy_woo_id=${row.legacy_woo_id} fam=${row.family} sub=${row.sub_category} sku=${row.sku} slug=${row.slug}`);
    ...
    // Resolve dedicated per-product video (matches fix-dedicated-video-urls.ts) on staging + public.
    console.log('\nResolving dedicated video URLs...');
    const stgVid = await client.query(
      `${DEDICATED_VIDEO_CTE}
       update legacy_import.stg_leftover_products s
    ...
    const pubVid = await client.query(
      `${DEDICATED_VIDEO_CTE}
       ...
       set video_url = d.video_url
       from dedicated d
       join legacy_import.stg_leftover_products s on s.legacy_woo_id = d.legacy_woo_id
    ...
    await client.query(
      `UPDATE public.product_import_batches
        ...
        WHERE id=$1`,
      [batchId, status, processedRows + failedRows, processedRows, failedRows, JSON.stringify({ phase: 'leftover-1', redirects_written: redirectInserted, warnings: Object.fromEntries(warningCounter) })],
    );
