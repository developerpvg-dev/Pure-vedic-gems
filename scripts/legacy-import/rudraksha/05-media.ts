/**
 * rudraksha/05-media.ts
 *
 * Phase A enumerates attachment IDs from stg_rudraksha_products into the shared
 * legacy_import.stg_media_url_map table. Phase B downloads only those Rudraksha
 * candidate URLs, converts them to WebP, and uploads them to Supabase Storage
 * under the rudraksha/ prefix.
 */
...
interface CliFlags { write: boolean; writeProd: boolean; enumerate: boolean; download: boolean }
...
async function main() {
  ...
  if (flags.download && flags.write && (!supaUrl || !supaKey)) {
...
async function loadCandidates(client: Client): Promise<MediaCandidate[]> {
  const products = await client.query(`
    SELECT legacy_thumbnail_url, legacy_image_urls
    FROM legacy_import.stg_rudraksha_products`);
...
async function enumerate(client: Client, flags: CliFlags): Promise<void> {
  ...
  const status = await mediaStatus(client, candidates.map((candidate) => candidate.legacy_url));
  console.log(`\nstg_media_url_map for Rudraksha after upsert (${processed} processed):`);
...
async function download(client: Client, flags: CliFlags, supaUrl: string, supaKey: string): Promise<void> {
  ...
  console.log(`Pending Rudraksha rows: ${pending.rows.length}`);
  ...
  async function worker(id: number) {
    while (queue.length > 0) {
      ...
      if (done % 50 === 0 || done === pending.rows.length) {
        process.stdout.write(`\r  [w${id}] processed ${done}/${pending.rows.length}  ok=${ok} dup=${skippedDup} failed=${failed}   `);
      }
    }
  }
...
async function processOne(
  ...
  const key = `rudraksha/${sha.slice(0, 2)}/${sha}.webp`;
