...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await admin.from('event_videos').update(updates).eq('id', id).select('*').single();

  if (error) return NextResponse.json({ error: 'Failed to update event video' }, { status: 500 });
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (error) return NextResponse.json({ error: 'Failed to deactivate event video' }, { status: 500 });
