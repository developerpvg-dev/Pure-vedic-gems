...
export async function GET() {
  ...
  const [{ data: categories }, { data: videos }] = await Promise.all([
    admin.from('event_video_categories').select('id, title, is_active, created_at').limit(500),
    admin.from('event_videos').select('category_id, is_featured, is_active, created_at').limit(5000),
  ]);
  ...
  for (const video of videoRows) {
    const key = video.category_id ?? 'unknown';
  ...
  return NextResponse.json({
    ...
    categoryBreakdown: catRows
      .map((row) => ({ label: row.title, value: videosByCategory.get(row.id) ?? 0, meta: 0 }))
      .sort((a, b) => b.value - a.value),
    statusBreakdown: [
      { label: 'Active videos', value: videoRows.filter((row) => row.is_active).length, meta: 0 },
      { label: 'Inactive videos', value: videoRows.filter((row) => !row.is_active).length, meta: 0 },
