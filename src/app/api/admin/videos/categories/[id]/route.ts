...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (error) return NextResponse.json({ error: 'Failed to update video category' }, { status: 500 });
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (error) return NextResponse.json({ error: 'Failed to deactivate video category' }, { status: 500 });
