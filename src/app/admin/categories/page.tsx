...
function categoryShopHref(category: Pick<ManagedCategory, 'source' | 'slug' | 'type' | 'family' | 'canonical_path'>) {
  const slug = slugify(category.slug);
  if (category.source === 'gem') {
  ...
  const legacySinglePath = `/shop/${slug}`;
...
export default function AdminCategoriesPage() {
  ...
  async function submitForm(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      canonical_path: form.source === 'catalog'
        ? form.canonical_path || categoryShopHref({ ...form, slug: form.slug || form.name })
        : form.canonical_path,
    };
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder={categoryShopHref({ ...form, slug: form.slug || 'category-slug' })}
                        className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500"
                      />
                    </div>
