/** ISO 3166-1 alpha-2 → alpha-3. PayGlocal billing wants alpha-3; our checkout stores alpha-2. */
const A2_TO_A3: Record<string, string> = {
  ...
  BI: 'BDI', KH: 'KHM', CM: 'CMR', CA: 'CAN', CV: 'CPV', KY: 'CYM', CF: 'CAF', TD: 'TCD', CL: 'CHL',
...
export function toAlpha3Country(input: string | null | undefined, fallback = 'IND') {
  const raw = String(input ?? '').trim();
  if (!raw) return fallback;
  const upper = raw.toUpperCase();
  if (upper.length === 3 && /^[A-Z]{3}$/.test(upper)) return upper;
  if (upper.length === 2 && A2_TO_A3[upper]) return A2_TO_A3[upper];
  return NAME_TO_A3[upper] ?? fallback;
}

export function splitPhone(phone: string | null | undefined) {
  const digits = String(phone ?? '').replace(/[^\d+]/g, '');
  const just = digits.replace(/\D/g, '');
  if (digits.startsWith('+1') && just.length >= 11) {
    return { callingCode: '+1', phoneNumber: just.slice(-10) };
  }
  if (digits.startsWith('+') && just.length > 10) {
    const codeLen = just.length - 10;
    if (codeLen >= 1 && codeLen <= 3) {
      return { callingCode: `+${just.slice(0, codeLen)}`, phoneNumber: just.slice(codeLen) };
    }
  }
  return { callingCode: '+91', phoneNumber: just.slice(-10) || just };
}
...
export function splitName(fullName: string | null | undefined) {
  ...
  return {
    ...
    lastName: parts.slice(1).join(' ') || 'PureVedicGems',
  };
}
