...
export const SCOPED_ROLE_DASHBOARDS: Partial<Record<CanonicalAdminRole, RoleDashboardConfig>> = {
  ...
  // ponytail: Website Maintenance uses full AdminShell (minus Settings) — too many pages for a scoped nav
  inventory: {
...
export function assertRoleDashboardAllowlists() {
  ...
  if (!isScopedRolePathAllowed('inventory', '/admin/leads')) throw new Error('inventory leads');
