...
export function ProductVideoReviewForm({
  token,
  orderNumber,
  videoUrl,
  round,
  ...
  const [done, setDone] = useState<{ status: 'approved' | 'changes_requested'; remarks?: string | null } | null>(
    () =>
      alreadyStatus
        ? { status: alreadyStatus, remarks: alreadyRemarks ?? null }
        : null,
  );
  ...
  const submit = async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/orders/product-video-review', {
        method: 'POST',
    ...
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <h2 className="mt-2 font-heading text-2xl text-stone-900">
          {done.status === 'approved' ? 'Thank you — design approved' : 'Changes noted'}
        </h2>
        <p className="mt-2 text-sm text-stone-600">
          {done.status === 'approved'
            ? 'We will continue crafting your piece with this design.'
            : 'Our team will review your notes and share an updated video when ready.'}
        </p>
      ...
      {decision === 'changes_requested' ? (
        <div className="mt-4">
          ...
          <textarea
            ...
            rows={4}
            placeholder="e.g. Stone orientation, metal finish, size…"
