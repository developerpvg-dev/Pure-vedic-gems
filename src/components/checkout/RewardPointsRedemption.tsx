...
function calculateMaxRedeem(rewards: CheckoutRewardState | null, subtotal: number) {
  ...
  const pointValue = Number(rewards.point_value_inr || 1);
  const maxByPercent = Math.floor((subtotal * (rewards.max_redeem_percent / 100)) / pointValue);
  const maxPoints = Math.max(0, Math.min(rewards.available_points, rewards.max_redeem_points_per_order, maxByPercent));
  return maxPoints >= rewards.min_redeem_points ? maxPoints : 0;
}
...
export function estimateRewardDiscount(points: number, rewards: CheckoutRewardState | null, subtotal: number) {
  ...
  return Math.round(Math.min(Math.max(0, points), maxPoints) * Number(rewards.point_value_inr || 1));
}
    ...
    <div className="pvg-checkout-step">
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-brand-muted">
              {rewards.available_points.toLocaleString('en-IN')} available. Earn {rewards.earn_points_per_order.toLocaleString('en-IN')} points after captured payment.
            </p>
          </div>
        </div>
        {discount > 0 && (
      ...
      {appliedPoints > 0 && (
        <p className="mt-3 text-xs text-brand-muted">
          {appliedPoints.toLocaleString('en-IN')} point(s) will be reserved when your order is created and confirmed after successful payment.
        </p>
      )}
    </div>
  );
