...
const STEPS = [
  { num: 1, label: 'Choose Gem', desc: 'Select from certified natural gemstones', icon: '💎' },
  { num: 2, label: 'Pick Design', desc: 'Browse rings, pendants, bracelets & more', icon: '✨' },
  { num: 3, label: 'Select Metal', desc: 'Gold, platinum, silver — your choice', icon: '🥇' },
  { num: 4, label: 'Certification', desc: 'GIA, IGI, GRS lab certification included', icon: '📜' },
  { num: 5, label: 'Energization', desc: 'Vedic puja and mantra energization', icon: '🙏' },
];
...
export function ConfiguratorCTA() {
  return (
    <section className="relative w-full overflow-hidden" style={{ background: 'var(--pvg-primary)' }}>
      ...
      <div className="relative grid grid-cols-1 md:grid-cols-[1fr_1fr_1.2fr]">
        {/* Left — Image showing gem-to-jewelry transformation */}
        <div className="relative hidden aspect-[3/4] md:block md:aspect-auto">
          <Image
        ...
        <div className="border-t border-white/10 p-6 md:border-l md:border-t-0 md:p-8 lg:p-10">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:gap-4">
            {STEPS.map((s) => (
              ...
              >
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[var(--pvg-accent)] text-lg">
                  {s.icon}
                </div>
                <div>
