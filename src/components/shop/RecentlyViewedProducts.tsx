...
export function RecentlyViewedProducts({ current }: { current: RecentlyViewedProduct }) {
  ...
  useEffect(() => {
    ...
    startTransition(() => setItems(stored.slice(0, SHOW_ITEMS)));
  ...
  return (
    <section className="mt-10 sm:mt-16">
      ...
      <ProductHorizontalScroller ariaLabel="Recently viewed products">
        {items.map((item) => (
          <Link
            key={item.id}
            href={item.href}
            role="listitem"
            className="group relative flex w-[42vw] max-w-[180px] shrink-0 flex-col overflow-hidden rounded-lg border border-black/[0.06] bg-white shadow-[0_2px_10px_rgba(61,43,31,0.06)] transition-shadow duration-300 hover:shadow-[0_6px_20px_rgba(0,0,0,0.10)] sm:w-[160px] sm:max-w-none md:w-[180px] lg:w-[200px]"
          >
            <div className="relative overflow-hidden bg-[#f2f2f2]" style={{ paddingBottom: '115%' }}>
              {item.imageUrl ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center text-xs text-brand-accent sm:text-sm">
                  PVG
                </div>
              )}
            </div>
            <div className="flex min-w-0 flex-1 flex-col px-2 pb-2 pt-1.5 sm:px-3 sm:pb-3 sm:pt-2">
              ...
              <p className="mt-1 text-[12px] font-semibold text-gray-900 sm:text-[14px]">
