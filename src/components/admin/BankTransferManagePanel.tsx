...
export function BankTransferManagePanel({
  ...
  async function run(action: 'verify_bank_transfer' | 'reject_bank_transfer') {
    ...
    if (action === 'verify_bank_transfer') {
      if (!confirm('Verify this bank transfer and confirm the order?')) return;
    ...
    try {
      ...
      if (action === 'verify_bank_transfer') {
        setMessage({ type: 'ok', text: 'Payment verified — order confirmed.' });
