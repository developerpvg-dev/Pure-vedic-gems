...
// Generated by ../_redirect_audit/build-p2-p11-redirects.js — do not edit by hand.
const p2p11Redirects = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'redirects/p2-p11-redirects.json'), 'utf8'),
) as [string, string][];
...
const nextConfig: NextConfig = {
  ...
  async redirects() {
    return [
      ...
      // P2–P11 leftover URLs (exact pairs so SEO audit clears). Specific rules above win first.
      // ponytail: no /blog|/videos catch-alls — those routes are live [slug] pages.
      // Blog: Sanity matches are skipped (live post→post); fetch via _redirect_audit/fetch-sanity-blog-slugs.js
      ...(p2p11Redirects.flatMap(([source, destination]) => [
        { source, destination, statusCode: 301 },
        { source: `${source}/`, destination, statusCode: 301 },
      ])),
